...
const SAFE_ARTIFACT_PATH_PATTERN =
  /^(?!\.)(?!.*(?:^|\/)\.{1,2}(?:\/|$))(?!.*\\)[^\u0000-\u001f\u007f]+$/;
...
function validateCoverPath(value) {
  const artifactPath = assertString(
    value,
    "submission.presentation.coverPath",
    { max: 300 },
  );
  invariant(
    SAFE_ARTIFACT_PATH_PATTERN.test(artifactPath) &&
      !artifactPath.startsWith("/") &&
      path.posix.normalize(artifactPath) === artifactPath,
    "submission.presentation.coverPath must be a normalized relative artifact path",
  );
  return artifactPath;
}
