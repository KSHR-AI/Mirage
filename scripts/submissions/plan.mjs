...
import { MAX_REGISTRY_BYTES, PLAN_SCHEMA_VERSION } from "./constants.mjs";
import { invariant, SubmissionError } from "./errors.mjs";
...
export async function createValidationPlan({
  ...
  assertRegistrySize(activeSubmissions);
...
export function validateSubmissionPlan(value) {
  const plan = assertPlainObject(value, "submission plan");
  assertExactKeys(
    plan,
  ...
  invariant(
    ...
    `submission plan schemaVersion must be ${PLAN_SCHEMA_VERSION}`,
  );
  invariant(
    plan.mode === "all" || plan.mode === "changed",
    "submission plan mode must be all or changed",
  );
  if (plan.mode === "changed") {
    assertCommit(plan.base, "submission plan base");
  } else {
    invariant(plan.base === null, "all-mode submission plan base must be null");
  }
  const head = assertCommit(plan.head, "submission plan head");
  invariant(
    Array.isArray(plan.submissions),
    "submission plan submissions must be an array",
  );
  invariant(
    Array.isArray(plan.removed),
    "submission plan removed must be an array",
  );
...
async function loadChangedSubmissions(base, head, cwd) {
  ...
  for (let index = 0; index < fields.length - 1;) {
    ...
    if (status === "A") {
      ...
      const current = await loadSubmission(head, firstPath, cwd);
      assertDeploymentOnlyUpdate(previous.submission, current.submission);
...
function assertDeploymentOnlyUpdate(previous, current) {
  const previousRecord = withoutField(previous, "deployment");
  const currentRecord = withoutField(current, "deployment");
  invariant(
    JSON.stringify(previousRecord) === JSON.stringify(currentRecord),
    `Accepted records are immutable except for deployment; ${previous.id} changes benchmark evidence`,
  );
}
...
async function loadSubmission(commit, filePath, cwd) {
  ...
  try {
    ...
    throw new SubmissionError(`Invalid JSON in ${filePath}: ${error.message}`);
  }
...
function validatePlanSubmission(value, index) {
  const label = `submission plan submissions[${index}]`;
...
function validateRemovedSubmission(value, index) {
  const label = `submission plan removed[${index}]`;
  const entry = assertPlainObject(value, label);
  assertExactKeys(
    entry,
...
function assertRegistrySize(values) {
  const games = values.map((value) => {
    const submission = value.submission ?? value;
    const record = withoutField(submission, "schemaVersion");
    return {
      ...record,
  ...
  invariant(
    ...
    `accepted submission registry exceeds ${MAX_REGISTRY_BYTES} bytes`,
  );
}
