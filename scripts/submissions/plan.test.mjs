...
import { createValidationPlan, validateSubmissionPlan } from "./plan.mjs";
import {
...
describe("submission validation plans", () => {
  it("plans every committed submission in all mode", async () => {
    ...
    expect(validateSubmissionPlan(plan).submissions).toHaveLength(1);
  });
  ...
  it("allows a verified deployment replacement without changing benchmark evidence", async () => {
    const fixture = await repositoryFixture();
    const added = await commitSubmission(fixture.root);
    const relocated = makeSubmission({
      deployment: {
        url: "https://night-drive.pages.dev/",
        provider: "Cloudflare Pages",
      },
    });
    await writeFile(added.filePath, `${JSON.stringify(relocated, null, 2)}\n`);
    git(fixture.root, ["add", "."]);
    git(fixture.root, ["commit", "--quiet", "-m", "Relocate deployment"]);
    const relocatedCommit = git(fixture.root, ["rev-parse", "HEAD"]);

    await expect(
      createValidationPlan({
        mode: "changed",
        base: added.commit,
        head: relocatedCommit,
        cwd: fixture.root,
      }),
      ...
      submissions: [
        expect.objectContaining({
          submission: expect.objectContaining({
            deployment: relocated.deployment,
          }),
        }),
      ],
    });
  ...
  it("rejects modification of accepted benchmark evidence", async () => {
    const fixture = await repositoryFixture();
    const added = await commitSubmission(fixture.root);
  ...
  it("rejects an active collection whose maximum public projection exceeds 256 KiB", async () => {
    ...
    ).rejects.toThrow(/accepted submission registry exceeds 262144 bytes/);
  });
});
